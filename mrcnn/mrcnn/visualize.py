...
def display_instances(image, boxes, masks, class_ids, class_names, scores=None, title="", figsize=(16, 16), ax=None, show_mask=True, show_bbox=True, colors=None, captions=None):
    ...
    for i in range(N):
        ...
        if not captions:
            ...
            caption = "{} {:.3f}".format(label, score) if score else label# 标签：准确率
        else:
            caption = captions[i]
        ax.text(x1, y1 + 8, caption, color='w', size=11, backgroundcolor="none")#标签及准确率的表头格式

        # Mask
        mask = masks[:, :, i]
        if show_mask:
            masked_image = apply_mask(image, mask, color)#每个实例掩盖处
...
def display_differences(image,gt_box, gt_class_id, gt_mask,pred_box, pred_class_id, pred_score, pred_mask, class_names, title="", ax=None, show_mask=True, show_box=True, iou_threshold=0.5, score_threshold=0.5):
